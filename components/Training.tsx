...
import { generateSubject, generateSubjectList, evaluateStudentWork } from '../services/geminiService';
      ...
      <div className="border-b border-slate-100 pb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600 mt-2">
            Générez des sujets, choisissez parmi des propositions, et recevez une correction immédiate.
          </p>
        </div>
        {step > 1 && (
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div>
            ...
            {exerciseType === ExerciseType.COMMENTAIRE && !selectedWork && (
               <p className="text-xs text-slate-500 mt-1">Si aucune œuvre n'est sélectionnée, un texte aléatoire hors programme sera généré.</p>
            )}
          </div>
          ...
          <div className="pt-4 flex flex-col sm:flex-row gap-4">
            ...
            >
              {exerciseType === ExerciseType.COMMENTAIRE 
                ? (selectedWork ? "Générer un Extrait de l'œuvre" : "Générer un Texte Aléatoire") 
                : "Générer un Sujet"}
            </Button>

            {selectedWork && (exerciseType === ExerciseType.DISSERTATION || exerciseType === ExerciseType.COMMENTAIRE) && (
              <Button 
        ...
        <div className="space-y-6 animate-fade-in">
          <h3 className="text-lg font-bold text-slate-900">
            {exerciseType === ExerciseType.COMMENTAIRE 
             ...
             <span className="block text-indigo-600 italic text-base font-normal mt-1">{selectedWork?.title}</span>
          </h3>
            ...
            {subjectList.map((subj, idx) => (
              ...
              >
                <div className="flex items-start">
                  <span className="bg-indigo-100 text-indigo-700 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-3 mt-0.5 group-hover:bg-indigo-600 group-hover:text-white transition-colors shrink-0">{idx + 1}</span>
      ...
      {step >= 2 && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-slate-50 p-6 rounded-lg border border-slate-200">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Sujet Sélectionné</h3>
